...
All five language bindings support the same router-based model:

| V5 Surface | Python | JavaScript | Go | PHP | Rust | Notes |
|---|---|---|---|---|---|
...
- Synchronous, blocking public API
- Public caller surface: `connect`, `ConnectOptions`, `Connection`, `Handle`
- Public service surface: `spawn`, `ServiceProcess`, `ServiceWorker`, `ServiceContext`, `runService`
- Uses `__call` magic for ergonomic method calling: `$handle->add(1, 2)`
