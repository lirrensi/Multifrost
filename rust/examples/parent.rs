...
async fn main() -> Result<(), MultifrostError> {
    ...
    let result: i64 = handle
        .call("add", vec![serde_json::json!(10), serde_json::json!(20)])
        .await?;
    println!("add(10, 20) = {}", result);

    // Call multiply
    let result: i64 = handle
        .call("multiply", vec![serde_json::json!(7), serde_json::json!(8)])
        .await?;
    println!("multiply(7, 8) = {}", result);

    // Call factorial
    let result: u64 = handle.call("factorial", vec![serde_json::json!(5)]).await?;
    println!("factorial(5) = {}", result);

    // Call fibonacci
    let result: u64 = handle
        .call("fibonacci", vec![serde_json::json!(10)])
        .await?;
